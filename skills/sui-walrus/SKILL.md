...
Targets: `@mysten/walrus` 1.2.21 (^1.1), `@mysten/sui` 2.27.1 (^2.27.1). Tested: 2026-08-29.

**Compatibility notes:** `@mysten/walrus@1.x` declares `@mysten/sui` as a peer dependency (`^2.27.1` at walrus 1.2.21). Do not install on top of a sui 1.x project — npm will pull a second sui copy and you will get dual-`SuiClient` type errors. The walrus JS SDK only works against `SuiGrpcClient` / `SuiJsonRpcClient` from sui 2.x. Run `npm ls @mysten/sui` first — if 1.x is present, decide before installing: upgrade the project to sui 2.x, or stay on legacy walrus tooling (CLI only).
