...
Targets: `@mysten/deepbook-v3` ^1.3 (V3 — current; the legacy `@mysten/deepbook` / `clob_v2` packages are deprecated and **not** what you want), `@mysten/sui` ^2.16. Last verified: 2026-05-21.
...
Prediction-market primitive built on DeepBook's CLOB — outcomes (binary markets, calls/puts) are tradable via Pool + BalanceManager, with a `Shared Vault` (PLP) providing liquidity and Block Scholes' OracleSVI pricing model on top.

**As of 2026-05, Predict is on testnet only** — `https://predict-server.testnet.mystenlabs.com`. Treat as experimental; verify mainnet availability with `sui_docs_query` before wiring it into production paths.
