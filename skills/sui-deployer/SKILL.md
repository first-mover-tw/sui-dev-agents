...
**Protocol 136 Notes (testnet v1.79.0 — NOT yet on mainnet; mainnet remains v1.78.1 / P135):**
- **`sui move test --package-size <MB>` (new flag, PR 27826):** raises/lowers the package arena size used by the test VM. The arena limit is now a protocol config (`package_arena_size_in_bytes`, default 10MB — same as the old hardcoded value, so no behavior change at the default). A package that exceeds it fails with `PACKAGE_ARENA_LIMIT_REACHED`; bump `--package-size` only for tests, not as a substitute for splitting an oversized package.
- Available from testnet v1.79.0 onward. Do not assume it exists on a `sui` binary pinned to 1.78.x.
