...
| `config_version<T>(enclave): u64` / `version<T>(config): u64` | Read the version an `Enclave<T>` was registered at vs the config's current version — detect enclaves left behind by a PCR rotation |
...
`MODULE_NAME`/`OTW_NAME` are your Move module name and one-time-witness type (e.g. `weather` / `WEATHER`). One `EnclaveConfig<T>` can back multiple `Enclave<T>` instances (different keys, same PCRs); register new ones at the latest `config_version` (compare `enclave.config_version()` against `config.version()` to spot stale ones).
