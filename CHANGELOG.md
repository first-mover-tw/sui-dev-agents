...
- **New freshness source kind: `endpoint`** — a marker read from one scalar in a live service's JSON, so a deployment can be watched alongside the repository that produces it. `memwal-relayer` (`relayer.memory.walrus.xyz/health` -> `build.commit`) now sits next to the `memwal` source, which watches upstream's own documentation on `dev`. The pair is the point: on 2026-09-04 they were 75 commits apart, and only the endpoint says which of the two a skill may describe in the present tense. `jsonPath` names a stable scalar deliberately rather than hashing the body — a whole-body marker would drift on every uptime counter — and it walks own properties only, refusing containers, so a marker can never become the constant `[object Object]` or a prototype method that is identical for every service.
- **New freshness source kind: `files`** — a marker built from the blob shas of named files at a ref, rather than the branch's HEAD sha. A HEAD marker drifts on every push; for a repo this plugin only *quotes*, most pushes touch nothing it cites, and each false drift costs a deep-investigation round. Blob shas are content identity, so a revert lands back on the previous marker instead of firing a second time, and the marker names which tracked file moved (`SKILL.md@51495f55 docs/llms.txt@102470ae ...`). Four outcomes are kept distinct: a network or auth failure on *any* path is a fetch error for the whole source — never a partial marker, which would drift once now and once more when the flaky path returns; a `404` is `EXTRACT_FAILED`, because a tracked file that was renamed or deleted means the source is watching nothing and must nag rather than retry in silence; a path that resolves to a directory or any non-sha answer is `EXTRACT_FAILED` for the same reason (the contents query tolerates a non-object reply at exit 0 instead of letting `jq` fail, which would be indistinguishable from a network error); and an empty or missing `paths` list is `EXTRACT_FAILED`, since an empty join is a marker that is stable forever.
...
- The freshness suite goes 41 -> 51 tests (10 new for `kind: 'files'`), mutation-tested with 7 mutants — the `404` check removed, the `404` check narrowed to stderr only, the fetch-error and extract-failure outcomes swapped, the blob-sha shape guard removed, the empty-`paths` guard removed, the configured `ref` dropped, and the per-path index frozen to the first path — each turning a *specific* test red rather than the suite.
...
### Changed
- **GitHub Actions runtimes moved off Node 20**, which the runners now force onto Node 24 while emitting a deprecation annotation on every job. `actions/checkout` v4 -> v7, `actions/setup-node` v4 -> v7, `actions/cache` v4 -> v6, and on the Pages workflow `configure-pages` v5 -> v6, `upload-pages-artifact` v3 -> v5, `deploy-pages` v4 -> v5. The four breaking changes across those majors are all no-ops here and were checked rather than assumed: `checkout@v7` refuses to check out fork PRs for `pull_request_target` / `workflow_run`, and both workflows trigger on `push` / `pull_request`; `setup-node@v5` caches automatically when `package.json` has a `packageManager` field and `@v6` narrows that to npm, while every job here already names `cache: 'npm'` explicitly; `upload-pages-artifact@v4` stopped including dotfiles, and `landing/` contains exactly one file, `index.html`. The five `setup-node` jobs also move from `node-version: '20'` (end-of-life since April 2026) to `'24'`, which is the version these gates and their self-tests are actually developed and mutation-tested on locally.
- **`memwal` stopped watching `dev`'s HEAD and now watches upstream's documentation blobs** — `SKILL.md`, `docs/llms.txt` and `.claude-plugin/marketplace.json`, i.e. exactly the three files `skills/sui-walrus/references/memory.md` points readers at. That reference was deliberately re-scoped to errata plus what upstream omits, so what this repo needs to hear about is upstream's docs changing, not upstream committing: `dev` moves dozens of commits between any change to the quoted material, and the deployment side of the story is already covered by `memwal-relayer`. The ref stays `dev` because it is the repo's default branch — what a reader following our links actually lands on.
- **`skills/sui-walrus/references/memory.md` re-scoped from "our MemWal reference" to "what upstream's own docs don't tell you".** MemWal now ships canonical agent-facing material of its own — a 587-line `SKILL.md` at its repo root and an official Claude Code plugin (`.claude-plugin/marketplace.json` → `memwal`, the MemWal MCP plus lifecycle hooks; `.agents/plugins/` for Codex) — so a hand-maintained second API reference in this repo would be a losing race against four package version lines. The file now opens with a table pointing at those sources and keeps only what they omit: errata, the deployed-vs-`dev` contract gap, and TypeScript detail verified against the published `.d.ts`.
- **Errata section added, because upstream's `SKILL.md` contradicts upstream's source.** It states namespaces have "no length cap, no character whitelist"; `services/server/src/types.rs` defines `MAX_NAMESPACE_BYTES = 255` on **both** `main` and `dev`, and `validate_namespace()` rejects empty and over-cap namespaces with HTTP 400. Upstream's `SKILL.md` was last touched 2026-08-22 and predates that work. Its `restore()` response-field table also omits the `truncated` flag that both branches return. This repo's own note was already correct on the cap — the errata records *why* it disagrees with the more official-looking source, so a future reader does not "fix" it back.
- **Corrected the deployment description.** The 2026-09-04 note called the production relayer *diverged* from `dev`; it is not. Upstream runs a `dev` → `staging` → `main` release train and `main`'s HEAD **is** the deployed commit (`559531fe`), ~75 commits behind `dev` — a release lag, not a fork. The distinction matters because "diverged" implies the deployment left the main line and its behaviour has to be reverse-engineered, whereas a lag means today's `main` is tomorrow's production. `/health` → `build.commit` remains the only thing that settles which contract you are on.
- **Four behaviours documented that upstream states and this file lacked**: namespaces are flat and opaque (exact-equality reads, no prefix or wildcard, no normalisation — `"my-app"` and `" my-app"` are different namespaces, and an omitted namespace becomes the literal `"default"`); `remember()` is always append and never upsert, so identical text written twice yields two entries that both surface on recall; isolation is enforced in the SQL `WHERE` clause, so cross-owner and cross-namespace rows are never decrypted or transferred; and `restore()` latency scales linearly with `limit` under fixed concurrency caps (10 Walrus downloads, 3 SEAL decrypts), seconds per blob cold, with `limit` capping the *inspected* set rather than `restored`.
- **Maturity re-stated from evidence rather than from the version number.** The relayer reports `apiVersion 1.0.0`, a `minSupportedSdk` floor, runtime `featureFlags`, and a `deprecations[]` list carrying `removalApiVersion` and migration guidance — the HTTP surface has a stated compatibility contract even though the TypeScript SDK is still `0.1.x`. Sibling packages recorded: `memwal-mcp` 0.0.11, the Python SDK 0.1.8 (published on PyPI as `memwal`, not under its source-directory name), `oc-memwal` 0.0.6.
