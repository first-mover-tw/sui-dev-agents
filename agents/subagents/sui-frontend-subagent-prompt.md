...
SDK: `@mysten/sui` (not `@mysten/sui.js`), `Transaction` (not `TransactionBlock`). v2 (Protocol 124):
...
- **API:** gRPC (GA, backend/low-latency/streaming default), GraphQL RPC (GA, frontend/indexer/complex queries), JSON-RPC (**shut off on public fullnodes** — permanently deactivated 2026-07-31). Balance API split (coinBalance/addressBalance).
